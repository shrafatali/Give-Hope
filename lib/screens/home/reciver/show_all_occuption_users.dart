...
class _ShowAllOccuptionUserState extends State<ShowAllOccuptionUser> {
  ...
  Widget build(BuildContext context) {
    ...
    return latLongC.text.isNotEmpty
        ? Scaffold(
            appBar: AppBar(
              centerTitle: true,
              leading: IconButton(
                        ...
                        return ListView.separated(
                            itemBuilder: (BuildContext context, int index) {
                              ...
                              if (documentData.isNotEmpty) {
                                ...
                                if (int.parse(documentData["availableItems"]
                                        .toString()) >
                                    0) {
                                  print("LATLONG : ${documentData["latLong"]}");
                                  List<String> latLong1 =
                                      documentData["latLong"]
                                          .toString()
                                          .split(",")
                                          .toList();

                                  print(
                                      "${double.parse(latLongC.text.toString().split(",")[0].trim())},${double.parse(latLongC.text.toString().split(",")[1].trim())},${double.tryParse(latLong1.toString().split(",")[0].trim())},${double.tryParse(latLong1.toString().split(",")[1].trim())}");
                                  if (Helper.calculateDistanceWithGeo(
                                          double.parse(latLongC.text
                                              .toString()
                                              .split(",")[0]
                                              .trim()),
                                          double.parse(latLongC.text
                                              ...
                                              .split(",")[1]
                                              .trim()),
                                          double.tryParse(latLong1[0].trim()),
                                          double.tryParse(
                                              latLong1[1].trim())) <=
                                      200000) {
                                    return FutureBuilder(
                                        future: getdata(
                                            documentData["userUID"].toString()),
